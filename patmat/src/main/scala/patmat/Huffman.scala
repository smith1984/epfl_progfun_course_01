...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def count(chars: List[Char], ls: List[(Char, Int)]): List[(Char, Int)] = {
      if (!chars.isEmpty) {
        count(chars.filter(_ != chars.head), (chars.head, chars.count(_ == chars.head)) :: ls)
      }
      else
        ls
    }
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith(_._2 < _._2).map(x => new Leaf(x._1, x._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    else if (!trees.isEmpty && !trees.tail.isEmpty)
